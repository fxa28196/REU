...
% ===========================================================================
% ORCA ACKNOWLEDGEMENT -- ONE-LINE TOGGLE
% ---------------------------------------------------------------------------
% NSF Award 2346732 funds the ORCA GPU accelerator. Acknowledging it is
% ACCURATE ONLY IF ORCA COMPUTE WAS ACTUALLY USED to produce results in this
% paper. As of 2026-08-04 it has NOT been used.
%
% BEFORE CAMERA-READY (2026-08-23), pick exactly one:
%   ORCA WAS used  -> leave the first line active, comment out the second.
%   ORCA NOT used  -> comment out the first line, uncomment the second.
% Nothing else in the document needs to change either way.
% ===========================================================================
\newcommand{\orcaack}{ Computational resources were provided in part by ORCA, supported by NSF Award No.~2346732.}
%\newcommand{\orcaack}{}
...
seeds, and a fourth, intake-rule scenario across three. The present 36-facility
system shelters 30.1\% of residents.
Raising capacity to match the population, without moving any facility, shelters
91.6\%; because capacity then equals population exactly, the arithmetic is
forced --- 578 spaces finish the event empty, equal to the 550 residents refused
everywhere plus 28 who cannot reach any shelter on the pedestrian-legal street
network. Splitting that identical capacity across ten additional sites --- more
doors, not better-chosen doors --- shelters 96.0\% and cuts refusals to 244; ten
sites drawn at random from the same candidate pool reproduce that count
run-for-run, so the headcount gain is attributable to dispersion, and siting
optimization earns credit only for shorter walks. In every scenario the residual
access gap concentrates in residents with mobility limitations, and the one
tested intervention that closes it is free: reserving 10\% of each facility's
spaces for mobility-limited arrivals moves the access gap from roughly
$+24$ to $0$ percentage points at identical total admissions. A capacity sweep
shows the gap is a knife-edge phenomenon of capacity equal to demand: 5\%
surplus already matches C's admissions (bounding C's siting advantage at
$\sim$342 spaces), and at 10\% surplus every reachable resident is admitted
and the gap vanishes. Compared against the one
(approximate) observed occupancy record, the model over-admits by a censored
bracket of 1.5--15.6$\times$, so all absolute access figures are upper bounds.}
...
``Unhealthy'' threshold for 194 of 312 hours. Those hours came in two spells:
a brief spike in the first day (hours 16--22 of the window), then 57 consecutive
hours of clean air, before the main episode arrived at hour 79 and held for
days.
...
Three responses are available, and they are not the same. The county could add
capacity, meaning more spaces inside the buildings it already operates. It
could split the same capacity across more sites. Or it could change the intake
rule at the door without touching a single building. These sound like versions
of one idea. They are not, and the differences matter both for how many people
get indoors and for which people do.
...
report that 550 people were refused shelter during an event in which the system
held exactly one space per person, or that the refusals concentrated in the
residents who walk slowest, which are the central results here.
...
the city's open data service \cite{portlandcampsites}; they resolve to 3,317
distinct coordinates in the file, and any single run seeds residents at a
sampled subset of them (2,981 in the seed-42 arm-A run). Two limitations matter. The feed
...
Street centerlines come from Oregon Metro's Regional Land Information System
(RLIS), a regional geographic data resource Metro publishes for public use, and
are redistributed as received with Metro's permission and credited to Metro as
their source. This work does not relicense them; the repository's license file
carves them out accordingly. This is also the one input whose retrieval date and
original release version were not recorded.
...
\noalign{\smallskip}\svhline\noalign{\smallskip}
Age 18--44 / 45--64 / 65+ (\%) & 52.7 / 42.3 / 5.0 & 52.8 / 42.0 / 5.2 & \cite{conte2026}, $N = 541$ pooled sample, local (M) \\
...
The street map becomes a network of 88,100 intersections and 109,434 segments,
...
\textbf{A defect in the street data.} During validation I found 25 intersection
identifiers each claimed by two or more different street features, at locations
1.65 to 18.5 km apart. The map file contains no legitimately multi-part shapes, so
...
coincident junction, covering 3 cases, or assigned a newly created intersection
at its true location, covering the remaining 22. After correction, the number of
...
\item \textbf{C, the same total, more doors.} Real facilities grow by only
...
\item \textbf{D, need-based intake.} Physically identical to B in every respect
--- the same 36 sites and the same 6,842 spaces --- but a fraction (10\% in the
headline runs) of each facility's spaces is reserved for mobility-limited
arrivals instead of being allocated strictly first-come, first-served. At a
reserve of zero, D reproduces B bit for bit.
\end{itemize}
...
measurements; the same street graph; the same parameters. Scenarios A, B and C
were each run with nine random seeds, 42 through 50, giving 27 runs; scenario D
was run at seeds 42 through 44. Changing the seed changes
...
4,754 residents are turned away because every facility they can reach is full,
...
3.04. Total modeled exposure falls 87.3\% and mean walking distance falls 56.7\%.
...
87.1\%, 86.9\%, and 87.1\%. These are not four independent confirmations of one
...
capacity, so the B$\rightarrow$C difference is attributable to how the same
total is split across sites; the random-pool control (see text) shows the
sheltered count is insensitive to \emph{which} additional sites are chosen.
The four exposure measures are not independent; see the text.}
\label{tab:outcomes}
\begin{tabular}{p{3.6cm}p{2.1cm}p{2.1cm}p{2.1cm}}
\hline\noalign{\smallskip}
 & A: today & B: more capacity & C: more doors \\
\noalign{\smallskip}\svhline\noalign{\smallskip}
Facilities & 36 & 36 & 46 \\
...
Mean inhaled dose ($\mu$g) & 23,373 & 3,056 & 1,536 \\
...
\caption{Access outcomes for the three siting scenarios: (a) residents reaching
a shelter, and (b) spare capacity and unmet need in the same run. Under scenario
B the system holds exactly one space per person and still strands 578 people
(550 refused at full facilities, 28 unable to reach any facility), because the
spaces are where the buildings are rather than where the people are. The
equality of empty spaces and unsheltered residents in panel (b) follows from
...
space per person. It still fails 578 of them.

That needs stating carefully, because a related observation is not evidence at
all. Scenario B leaves 578 spaces empty and fails to shelter 578 people, of
whom 550 were refused at a full facility and 28 could reach no facility. Those
totals are equal by arithmetic, not by discovery: when capacity equals
population, empty spaces must equal unsheltered people. The near-equality
proves nothing on its own. What carries weight is that anyone was refused.
Supply matched demand exactly, and 550 people were still turned away at door
after door, because the added capacity went to buildings that already exist
rather than to the places where people are. That is a failure of geography, not
of supply, and the design separates the two cleanly, because capacity is not
the binding constraint in B. It is also a knife-edge failure: a capacity sweep
at the same 36 sites shows that at $1.1\times$ demand --- 684 spaces of
surplus --- every reachable resident is admitted and the failure mode
disappears; at $1.05\times$ (342 spaces) admissions already match scenario
C's, bounding the siting advantage at about 342 spaces
(Sect.~\ref{sec:equity}).

Scenario C spends the identical 6,842 spaces, split across ten additional
sites. Refusals fall from 550 to 244 and empty spaces from 578 to 272. Mean
walking distance falls a further 25\%, total modeled exposure a further 50.7\%,
and mean inhaled dose from 3,056 to 1,536 $\mu$g. Fig.~\ref{fig:map} shows the
spatial arrangement. Measured against today's system, C shelters 3.19 times as
many residents, reduces total modeled exposure by 93.7\%, and mean distance
walked by 68\%.

The attribution matters, and a control settles it. Ten sites drawn uniformly at
random from the same 498-node candidate pool the optimizer searched reproduce
C's sheltered count exactly, in every seed tested (42, 43 and 44). The
headcount gain over B is
therefore \emph{dispersion} --- the same total split across more doors --- and
not the optimizer's choice of doors; siting optimization earns its credit on
walking distance alone, and that credit is conditional on residents knowing
where every facility is. Capacity remains the first-order effect. What a model
of individual movement adds is the ability to show that identical total
capacity produces different access depending on how it is split across doors
--- a static screening index cannot show that, because none of its inputs
change when a facility fills. A min-cost-flow bound confirms the admissions
count in C is already at the coordination optimum for its site set, so the
remaining lever is not further siting optimization but the intake rule
(Sect.~\ref{sec:equity}). None of this shows that these are the refusal counts
a real expansion would produce.

\begin{figure}[t]
\includegraphics[width=\textwidth]{fig4_map}
\caption{Resident start points, in gray, against facility locations, sized by
capacity, in (a) today's system and (b) the more-doors scenario C. In (a) the
...
\begin{table}[t]
\caption{Percentage of each group reaching a shelter, seed 42. D is scenario B
with a 10\% per-facility reserve for mobility-limited arrivals; its total
admissions equal B's.}
\label{tab:groups}
\begin{tabular}{p{3.4cm}p{1.3cm}p{1.2cm}p{1.2cm}p{1.2cm}p{1.2cm}}
\hline\noalign{\smallskip}
Group & Share (\%) & A (\%) & B (\%) & C (\%) & D (\%) \\
\noalign{\smallskip}\svhline\noalign{\smallskip}
...
expansion widens the gap from 12.5 to 23.7, and dispersion returns it to 12.5.
On a ratio scale the gap narrows at every step, from 1.62 to 1.33 to 1.15.
Counting people rather than rates, the number of mobility-limited residents
left outside falls from 1,087 to 373 to 190. Sex is reported for completeness:
the male--female access difference is small relative to the mobility gap (at
most a few percentage points, in scenario B).
...
These measures disagree because of a ceiling effect. At 96.3\% access in scenario
...
mobility limitations, from 20.1\% to 72.6\% to 86.0\%. Capacity expansion is not
harmful to this group; it helps them substantially. Second, residents with
mobility limitations remain the worst-served group under every siting scenario
and are over-represented among those still outside. They are 19.9\%
of the population but 23\% of the residents left outside today, 65\% under
capacity expansion, and 70\% under dispersion. As the system improves
...
Third, the intake rule --- not siting --- is what closes the gap. Scenario D
reserves 10\% of each facility's spaces for mobility-limited arrivals and moves
their access from 72.6\% to 91.9\%, above the unimpaired rate of 91.5\%, at
total admissions identical to B's and zero capital cost. Under D the race
dynamic that produces the gap is simply switched off for the reserved spaces:
survival analysis shows that in B roughly 80\% of the final mobility gap
already exists one hour after departure (Fig.~\ref{fig:race}), so any rule that
must be won by arrival order will be won by the fast.

Two further results bound the finding, and both contradicted this study's own
registered predictions, which is reported here deliberately. A capacity sweep
at B's real sites shows the mobility gap is a property of the scarcity band
where capacity approximately equals demand: $1.05\times$ demand (342 surplus
spaces) already matches C's admissions --- without closing the gap (15.0 pp)
--- and at $1.1\times$ (684 spaces) every reachable resident is admitted and
the gap vanishes. We predicted the crossing would require $1.4$--$1.6\times$
and that the gap would persist under surplus; both predictions were wrong, and
the honest conclusion is stronger --- modest surplus dissolves the failure
mode, C's siting advantage on headcount is bounded at about 342 spaces, and
the triage reserve matters exactly when surplus cannot be bought.

\begin{figure}[t]
\includegraphics[width=\textwidth]{fig5_race}
\caption{The mobility gap is a first-hour race, and the intake rule ends it.
Cumulative share of each group already inside a shelter, by minutes since
departure (seed 42). Left: under scenario B, the curves separate within the
first hour and never re-converge. Right: under scenario D's 10\% reserve, the
mobility-limited curve reaches the unimpaired curve --- same buildings, same
spaces, different rule.}
\label{fig:race}
\end{figure}

The same broad pattern holds for residents aged 65 and over, whose access moves
from 22.4\% to 81.6\% to 90.1\%, and for residents with COPD, from 22.6\% to
87.3\% to 95.1\%. Asthma shows almost no access penalty while COPD shows a large
...
is 306. The two numbers are reported side by side rather than as a ratio: seed
spread bounds only Monte-Carlo variability, not structural or parametric
uncertainty. No range overlaps between scenarios on any headline metric, so the
ordering A $<$ B $<$ C is not an artifact of any particular random draw; the
B--C gap in particular is reproduced exactly by the random-pool control, so its
tightness is evidence about dispersion, not the optimizer. A stronger
robustness result arrived from a correction: after freeway-class street
segments (2,636 features, 614~km, including two non-pedestrian bridges) were
removed from the walking graph and every run regenerated, all sheltered counts
were unchanged to the digit in every scenario and seed.

One diagnostic falls out of the design. The count of residents who can reach no
shelter at all is identical across all three scenarios within each seed --- 28
at seed 42, verified as the same individuals --- because these residents sit on
street-graph fragments whose only connection to the network was a freeway.
No amount of capacity and no re-placement reaches them, and the number varies
only with which residents were sampled. This confirms the scenarios differ only
in the way intended. It is not a check on external accuracy.
...
\textbf{Admission depends on arrival order, and that is now a tested lever
rather than an untested caveat.} Residents are served in shuffle order rather
than by need. This matters most in scenario A, where capacity binds hardest,
and it is the mechanism producing the concentration of mobility-limited
residents among the excluded that Sect.~\ref{sec:equity} reports. Scenario D
tests the needs-based alternative directly: a 10\% per-facility reserve for
mobility-limited arrivals closes the gap at identical total admissions, so the
equity findings for A--C should be read as conditional on
first-come-first-served admission, with D quantifying exactly how much of the
gap that rule creates.
...
gradients the data cannot support; one consequence is that siting can help
...
\textbf{The behavioral scope is deliberately minimal, and each simplification
is one-directional.} Every resident knows every shelter and walks to it:
during the real 2020 event access was partly dispatcher-mediated, with
transportation arranged through 211, and neither information limits nor rides
are modeled. Residents have no destination other than a shelter --- no
self-rescue, no leaving the county. The model is adults-only by construction,
which happens to match the 2020 emergency sites' adults-only intake, though
not the general shelter system. The historical 2020 configuration also held
Mount Scott Community Center in reserve as a standby site; conditional
activation of standby capacity is not modeled. Scenario B's uniform scale-up
is physically unrealizable for much of the inventory (motel shelters would
need to triple their rooms; the largest pod village would grow from 66 to 202
pods) --- B is a diagnostic bound, not a plan. Finally, an asymmetric
evidentiary bar is acknowledged: ten real day centers are excluded for lack of
a published capacity while scenario C's ten hypothetical sites carry invented
349-space capacities; the two choices are stated together so the asymmetry is
visible.
...
The answer to the first is yes, with the attribution the controls force. The
present system shelters 30.1\% of residents. Raising capacity to match the
population shelters 91.6\% and, holding exactly one space per person, still
strands 578 of them (550 refused everywhere, 28 unable to reach any facility).
Splitting the identical capacity across ten additional sites shelters 96.0\%
and cuts refusals to 244 --- a gain the random-pool control shows is
dispersion, not optimized siting. The ordering held across nine random seeds
with no overlap between scenarios on any headline metric, and every sheltered
count survived a full re-run on the corrected pedestrian-legal street graph
unchanged.

The answer to the second is yes, and it is the finding with no budget attached.
On a percentage-point scale, capacity expansion widens the mobility gap from
12.5 to 23.7 points and dispersion returns it to 12.5; on a ratio scale, both
narrow it. What holds regardless of scale is that residents with mobility
limitations remain the worst-served group throughout. The intervention that
closes the gap is the intake rule: reserving 10\% of each facility's spaces for
mobility-limited arrivals takes the gap to zero at identical total admissions
and zero capital cost. A capacity sweep sharpens the interpretation --- and
overturned this study's own registered predictions: 5\% surplus over demand
(342 spaces) already matches C's admissions without closing the gap, and at
10\% surplus (684 spaces) every reachable resident is admitted and the
mobility gap vanishes entirely --- so the gap is a phenomenon of the scarcity
band where capacity approximately equals demand, and C's siting advantage on
headcount is worth at most about 342 spaces. The policy sentence that survives every control is: buy surplus if you
can; where you cannot, the triage reserve buys the same equity for free.

Three extensions would improve the model most. A model of awareness is the most
...
described in Sect.~\ref{sec:equity}. Third, the placement
...
hosted by Portland State University under NSF Award No.~2244551.\orcaack{} I
thank Prof. Christof Teuscher for his mentorship and guidance throughout the
program. Street centerline data was provided by Oregon Metro through its
Regional Land Information System, used here with Metro's permission.
...
The street centerline file is Oregon Metro RLIS data, redistributed as received
with Metro's permission and credited to Metro; the repository's license file
carves it out rather than relicensing it.
...
\bibitem{conte2026}
Kathleen Conte, Amanda Laird, Danielle DuBoise, Sonia Avila, Corinne Bone, TREES
Committee, and Marisa Zapata. \emph{The Pathways Study: Findings from Surveys of
People with Recent Experience of Homelessness in Multnomah County}. Portland
State University Homelessness Research and Action Collaborative, 9 April 2026.
Homelessness Research and Action Collaborative Publications and Presentations,
55. \url{https://archives.pdx.edu/ds/psu/44627}.
