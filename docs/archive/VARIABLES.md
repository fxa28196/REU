...
# Scientific Variable Registry â€” Wildfire Smoke Shelter ABM

Living registry of every scientific variable in the model. Numbering follows
`PROJECT_ASSESSMENT.md` Phase 3. Rules: **no invented values** â€” every number
...
Format per variable: purpose Â· implementation location Â· scientific mechanism
Â· assumptions Â· limitations Â· source.

---

## V13 â€” `minutesPerTick` (tick â†” wall-clock mapping)

- **Purpose:** Defines the simulated duration of one Repast schedule tick;
  the Î”t underlying every future exposure integral (exposure-hours, VWE) and
  the alignment of the run to the Sept 7â€“19, 2020 hourly PM2.5 record.
...
- **Scientific mechanism:** With 1 min/tick, 60 ticks aggregate to one hourly
  ...
  remainder; the Sept 7â€“19 2020 event (12 days) spans 17,280 ticks.
...
- **Limitations / rationale for value:** This is a **modeling decision, not a
  ...
  because a 5-min tick at 1.3 m/s walks ~390 m/tick â€” longer than many street
...
## V10 â€” `walkingSpeedMps` (mobility)
...
- **Implementation:** `Geography.rs/parameters.xml` (`walkingSpeedMps`,
  ...
  (`stepLengthM = walkingSpeedMps Ã— 60 Ã— minutesPerTick`).
- **Scientific mechanism:** Comfortable (self-selected) gait speed of adults;
  replaces the stock demo's uncited anisotropic 0.00015 Â°/tick (~12â€“17 m/min
  depending on heading at 45.5Â° N).
- **Source:** Bohannon RW (1997). *Comfortable and maximum walking speed of
  adults aged 20â€“79 years: reference values and determinants.* Age and Ageing
  26(1):15â€“19. **DOI: 10.1093/ageing/26.1.15**. Reported mean comfortable
...
## â€” `shelterArrivalDistanceM` (arrival threshold)
...
- **Scientific mechanism:** Threshold replacing the stock demo's 0.002-degree
  radius, which was direction-dependent at Portland's latitude (~222 m Nâ€“S vs
  ~156 m Eâ€“W); 200 m is the same order of magnitude, now isotropic.
...
## V9 â€” `distanceTraveledM` (derived)
...
- **Scientific mechanism:** Derived quantity â€” no external value. Geodesic
...
## V11 â€” `networkDistToShelterM` (shelter accessibility)

- **Purpose:** Street-network distance (metres) from a resident's starting
  node to the shelter it selects â€” "the nearest shelter you can *actually
...
- **Implementation:** `GisAgent.getNetworkDistToShelterM()`; computed at
  ...
  against which walked distance is checked (walked â‰¤ planned +
...
- **Scientific mechanism:** Shortest-path distance over the RLIS street
  ...
  geodesic polyline lengths in metres). Undirected graph â€” pedestrians are
...
- **Limitations:** Freeway/highway segments (CFCC codes) are not yet
  filtered from the pedestrian graph â€” flagged for the data-curation pass
  (roadmap commits 6â€“7). NaN when no shelter is reachable (disconnected
...
- **Source:** derived quantity; graph algorithm standard (Dijkstra 1959,
  Numer. Math. 1:269â€“271, DOI: 10.1007/BF01386390).

## Outcome states â€” `GisAgent.State` (commit 5)
...
- **Implementation:** `GisAgent.getState()`, `getArrivalTick()`,
  `getTargetShelter()`; agents are **never removed from the context** â€” a
...
- **Scientific mechanism:** Outcome measurement requires the full
  ...
  exposure statistic toward survivors (survivorship bias â€”
...
- **Source:** design requirement from the project brief (Agent ID /
  Final outcome record) â€” no external value.

## Movement-geometry implementation note (commit 3)

All movement arithmetic in `GisAgent.step()` â€” shelter-distance ranking, path
orientation, vertex advance, per-tick displacement â€” now uses GeographicLib's
...
Karney 2013, *Algorithms for geodesics*, J. Geodesy 87:43â€“55,
...
1. **Nearest-street selection** still ranks candidate streets by JTS planar
   ...
   (cos 45.5Â° â‰ˆ 0.70 between Eâ€“W and Nâ€“S degree lengths) can misrank streets
...
2. **Vertex overshoot:** an agent advances its path index when the next
   vertex is closer than one tick's step length, then walks a full step â€”
