# Phase 2 Â· Spec 7 â€” Output Redesign

**Status: IMPLEMENTED.** Extends the data dictionary in `METRICS.md` without
...
of â‰ˆ0.63 from a superseded run (the baseline is 0.82).

**Correct `METRICS.md` before extending it.** A data dictionary that misnames its
own columns is worse than none â€” it is the document an external reviewer would
trust.
...
**`age`, `asthma`, `copd` already exist in the header and are written empty** â€”
...
simulation ID, commit hash, dataset versions â€” the last four already exist as
...
- **`network_dist_to_shelter_m`** is set once at first selection. Under
  ...
  `walked â‰ˆ snap + network_dist`. **Freeze the column's meaning** as
...
| File | Shape | Purpose |
|---|---|---|
| `summary.md` | human executive summary | arrival rate, exposure headlines, highest-risk group, scenario delta from baseline â€” **with the `METRICS.md` interpretation guardrails reproduced verbatim**, so no number is quoted without its caveat |
...
| `decisions.csv` (optional, off by default) | `agent_id, tick, action, chosen_shelter, utility_termsâ€¦` | behavioural audit trail; gated because 2,000 agents Ã— many events is large |
...
1. **Executive summary** â€” what was run (scenario, seed, n), what happened
   (arrival rate, median/max travel, exposure median and Gini), and the single
   most important finding, in prose.
2. **Demographic breakdown** â€” realised composition vs the specified source
   distribution, so a reader can see the sampler behaved.
3. **Vulnerability breakdown** â€” counts per susceptibility class.
4. **Shelter utilisation** â€” occupancy, unused capacity, refusals, arrival
   timing per shelter.
5. **Exposure statistics** â€” distribution, **stratified by susceptibility
   class** (the primary scientific output per `02-VULNERABILITY.md` Â§5).
6. **Highest-risk groups** â€” Â§4.1.
7. **Why those groups were affected** â€” Â§4.2.
8. **Limitations** â€” inherited verbatim from the specs, never re-worded per run.

### 4.1 Highest-risk attribution must be defensive

Report the group with maximum mean exposure **subject to `n â‰¥ minGroupSize`**
...
The explanatory sentence then writes itself from the numbers â€” "agents with
mobility limitation accrued 38% more dose because their mean time outdoors was
1.9 h longer at the same network distance" â€” and every clause traces to an
...
`scenario` (`06-SCENARIOS.md` Â§3); `rng` (stream names, derived seeds,
derivation function); `hazards` (event count, closed-edge-hours, agents
affected); `population.attributes` (realised vs specified distribution â€” the
...
- `vwe == exposure` â†’ assert
  `|vwe âˆ’ exposure Ã— age_rr Ã— comorbidity_rr| < 1e-6`. Since both RRs are
  ...
  important test change in the plan** â€” strictly stronger than what it replaces.
- `all RR == 1.0` â†’ assert **iff** the scenario disables vulnerability
  weighting.

Also required:

- **`test_routing.py` T3 breaks by design** under heterogeneous speed: the global
  1.20â€“1.40 m/s band becomes a per-agent check against the new
...
- The shelter-utilisation figure computes capacity as float and breaks on an
  operating shelter with **empty** capacity â€” the exact shape of the standby
