# Phase 2 · Spec 5 — Dynamic Hazard System
...
and a recovery assumption. For the one hazard that would most change results —
urban road/path closure — **the evidence needed to specify those three things
...
|---|---|---|---|---|---|
| **Urban road / path closure (smoke-caused)** | **No documented occurrence in Portland, Sept 2020** | — (see §2) | — | ODOT/press closure records for the event show OR-22, US-20 (MP 33–72), OR-126 (MP 10–55), I-5 near Ashland — **all fire-corridor closures far outside the study area** | **OMIT** |
| **Shelter saturation / turn-away** | Emergent from hard capacity (99/site) + arrivals | n/a — a state, not an event | Bed frees on departure (no departures modelled in the event window) | **VERIFIED**: county releases (capacity 99 each); Street Roots observed 40 and ~90 occupants; **no documented capacity turn-away** | **KEEP — emergent, not exogenous** |
| **Shelter opening / closing schedule** | Deterministic, historical | 9-day operation | n/a | **VERIFIED**: OCC opened Sept 10, Charles Jordan Sept 11, both through Sept 18/19 | **IMPLEMENT — real, and currently missing** |
| **Smoke intensity change** | Already measured hourly (EPA AQS) | Empirical series | Empirical | **VERIFIED**: Oregon DEQ 2020 Wildfire Smoke Trends — worst air quality since monitoring began in 1985; Portland metro **8 days Very Unhealthy/Hazardous Sept 10–17, 5 consecutive days ≥ 250.5 µg/m³** | **KEEP measured; scenarios use real alternative episodes (§3)** |
| **Information failure (agent unaware)** | **p ≈ 0.65 unaware** | Persistent unless word-of-mouth enabled | n/a | **VERIFIED**: PSU survey n=73 (`04-DECISION.md` §2.1) | **IMPLEMENT — the best-evidenced disruption available** |
...
**Duration.** The distributional *family* is well established — incident-duration
...
start_tick, end_tick, cause`), label the whole scenario **SYNTHETIC — class A**,
...
## 3. Smoke scenarios — real episodes, not multipliers

A scalar "×1.5 smoke" multiplier has **no basis in the literature**, and it is
...
1. **Real alternative episodes from EPA AQS** — e.g. Portland's shorter/lower
...
2. **If a synthetic stress test is required:** perturb **duration** (extend the
   ≥ 250.5 µg/m³ plateau), not amplitude — duration is the empirically varying
   dimension across episodes. Label **SYNTHETIC**.
3. **Amplitude multiplier — not recommended.** If used at all, label it
...
rather than noise — Glasserman & Yao 1992, *Management Science* 38(6):884–908,
...
sampling (`08-ENGINEERING.md` §3); paired differences across ≥ 30 replications.
...
1. **Shelter opening/closing schedule** (deterministic, verified) — alone this
   fixes the standing AUDIT #1 artefact, and it is pure evidence.
2. **Capacity saturation as an emergent state** — partly present; needs
   population scaled above 198 to bind, plus order-independent admission so the
   last bed is not awarded by RNG shuffle order (`08-ENGINEERING.md` §3.5).
3. **Information failure** — the awareness stage in `04-DECISION.md`.
4. **The hazard *mechanism*** (edge mask, masked Dijkstra, reroute-on-encounter)
   ...
   Architecture: `08-ENGINEERING.md` §4.
