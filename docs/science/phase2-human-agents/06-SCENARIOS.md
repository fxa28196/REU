# Phase 2 · Spec 6 — Scenario Framework

**Status: DESIGN ONLY.** Requirement: run baseline, high-smoke, extra-shelters,
...
|---|---|---|---|---|
| **A. Many flat Repast parameters** | Yes, natively | **No** — Repast parameters are scalars; shelter sets and closure lists are inexpressible | automatic if the manifest enumerates the schema | Rejected as sole mechanism |
| **B. One `scenario` id parameter → external config file** | Yes (`type="list" value_type="string"`; parameters nest, so scenario × seed is one sweep file) | Yes, via file-pointer keys | needs explicit dump of the resolved config + file SHA-256 | **RECOMMENDED** |
...
## 2. Recommended design — one parameter, three resolution layers

**Add exactly one Repast parameter** in `Geography.rs/parameters.xml`:
`scenario` (String, default `baseline`).

**Do not edit `Geography/batch/batch_params.xml`** — it is the archived baseline
...
`java.util.Properties` — JDK-only, comment-friendly, diff-friendly. Keys are
namespaced; **every key has a code default equal to today's behaviour**:

```properties
# high-smoke.properties  — prefer the real-episode variant (05-HAZARDS §3)
...
`baseline.properties` contains only a `description` line — so `scenario=baseline`
...
3. Repast parameters — the existing scalars always win, so a `walkingSpeedMps`
...
only files actually referenced** — `baseline.properties` references none, so the
...
|---|---|---|---|
| `baseline` | none | — | must stay byte-identical to `docs/runs/final-baseline/` |
| `high-smoke` | alternative **real** episode; or duration-extended plateau | **M** (real) / **A** (synthetic) | amplitude multipliers discouraged — `05-HAZARDS.md` §3 |
...
| `reduced-mobility` | mobility-limited share → 0.27; speed distribution shifted | **M/L** | uses the upper bound of the verified 0.19–0.27 range |
| `road-disruption` | explicit deterministic closure list | **SYNTHETIC / A** | conditional experiment only, never probabilistic — `05-HAZARDS.md` §2 |
| `capacity-shortage` | `numAgents` > 198 so 2×99 binds | **M** (capacity) + **A** (scale) | requires order-independent admission first — `08-ENGINEERING.md` §3.5 |
...
- **Common random numbers across scenarios** (`08-ENGINEERING.md` §3.4): same
  seed ⇒ same population, same attributes per agent id, same awareness and
...
- **Paired differences** against baseline on: arrival rate, travel
  ...
  Unhealthy, per-shelter occupancy and refusals — the protocol already written in
  `BASELINE_RESULTS.md` §7.
- **Replications:** ≥ 5 seeds for any quoted scenario result; ≥ 30 for
