# Phase 2 Â· Spec 4 â€” Shelter Decision Model

**Status: IMPLEMENTED.** Formally reopens `DESIGN_SPEC.md` Part III **F3**
("Behavioural heterogeneity â€” no data source identified; would be pure
...
bibliography â€” a Portland State University survey of unhoused people about the
...
Today every agent evacuates the instant county PM2.5 crosses 55.5 Âµg/mÂ³, walks
...
*Risk Analysis* 32(4):616â€“632,
DOI [10.1111/j.1539-6924.2011.01647.x](https://doi.org/10.1111/j.1539-6924.2011.01647.x)),
this omits the entire **pre-decision stage** â€” exposure to information â†’
attention â†’ comprehension must precede any protective action. For this
...
### 2.1 Awareness â€” the one directly sourced behavioural parameter
...
â†’ **p_aware â‰ˆ 0.35** (class **L**; local and event-specific).
...
sample). Sensitivity range **0.25â€“0.45**.

### 2.2 Calibration targets â€” what actually happened

| Fact | Source |
|---|---|
| Oregon Convention Center opened **Sept 10**, Charles Jordan CC **Sept 11**, both **99-person capacity** (COVID-constrained), third site standby, operating **9 consecutive days** through Sept 18/19 | Multnomah County Joint Office of Homeless Services news releases, Sept 10â€“18 2020 |
| One observed night (Sept 16): **~90 at OCC, 40 at Charles Jordan** â€” ~130 of 198 beds â€” against **~2,000 sleeping unsheltered countywide** | Street Roots, 16 Sept 2020 |
...
**Derived uptake â‰ˆ 130/2,000 â‰ˆ 6â€“7% per night.** This is a **derivation from two
documented numbers, not a published statistic** â€” cite it that way. With 35%
awareness it implies conditional uptake given awareness of roughly 15â€“20%.

These two occupancy numbers (â‰ˆ90/99 and 40/99) are the **only quantitative
...
consistently documented â€” Every & Richardson 2019 (AJEM); Gin & Dobalian
(*Natural Hazards Review*, DOI 10.1061/NHREFO.NHENG-2356); Settembrino
(Hurricane Sandy Quick Response report) â€” but **never parameterised**.

**Design consequence:** do not invent separate sub-parameters for pets,
belongings, and distrust. Fold them into one latent "reluctant" class (Â§3.2).
...
### 3.1 Stage 0 â€” Information (PADM pre-decision) Â· class **L**

```
A_i(0) ~ Bernoulli(p_aware),    p_aware = 0.35        [PSU survey]
```
An unaware agent cannot select a shelter. Optional word-of-mouth growth
`dp/dt = ÎºÂ·(contacts)` with **Îº = 0 by default** (class **A**; sensitivity
scenario only).

### 3.2 Stage 1 â€” Whether and when to go Â· form class **L**, coefficients class **C**

Sequential (repeated) binary logit, one decision per period, in the established
evacuation-demand form of Fu & Wilmot 2004 (*TRR* 1882:19â€“26,
DOI [10.3141/1882-03](https://doi.org/10.3141/1882-03)):

```
P_i(go at t | aware, not yet gone)
    = 1 / ( 1 + exp( âˆ’( Î²â‚€ + Î²â‚Â·ln C(t) + Î²â‚‚Â·D(t) + Î¸_i ) ) )
```

- `C(t)` â€” current PM2.5 (environmental cue).
- `D(t)` â€” cumulative days at Hazardous levels (PADM cue accumulation; the
  Sept 2020 episode ran **5 consecutive days â‰¥ 250.5 Âµg/mÂ³**, so duration is a
  real, distinguishing cue).
- `Î¸_i` â€” latent-class intercept over {shelter-inclined, reluctant}, share Ï€.
  Latent-class structure is supported by McCaffrey, Wilson & Konar 2018
  (*Risk Analysis* 38(7):1390â€“1404, DOI 10.1111/risa.12944), which identifies
...
**Î²â‚€, Î²â‚, Î²â‚‚, Ï€ are class C (calibrated), not L.** Fit them so the model
reproduces the two local targets â€” steady-state uptake â‰ˆ 6â€“7%, per-site
occupancy bracketing 40/99 and ~90/99 â€” then report target, fit, and residual as
`DESIGN_SPEC.md` requires for class C, and sweep each Â±50%.

**Keep 55.5 Âµg/mÂ³, but move it to where it belongs:** it becomes the
...
```
Ï„_i ~ Lognormal(Âµ, Ïƒ)
```
Right-skewed positive families (lognormal, log-logistic, gamma, Weibull) are the
established choice for pre-movement delay â€” Lovreglio, Kuligowski, Gwynne &
...
only the family transfers, never its parameters.** Default median 1â€“6 h, swept,
consistent with documented multi-day "toughing it out".

### 3.3 Stage 2 â€” Where to go Â· form class **L**, coefficients class **A**

Multinomial logit over shelters known, open, and within walking range R:
```
U_ij = âˆ’Î²_d Â· d_ij + Î²_o Â· OPEN_j(t) + Îµ_ij ,    Îµ ~ Gumbel
P_ij = exp(U_ij) / Î£_k exp(U_ik)
```

- **Negative distance coefficient** â€” sign and MNL structure are well
  established: Cheng, Wilmot & Baker 2008 (hurricane evacuation destination
  choice, Hurricane Floyd survey) estimate âˆ’0.004655 per mile (friends/
  relatives) and âˆ’0.005882 per mile (hotel/motel), with positive capacity-proxy
  and negative in-hazard-path coefficients. **These magnitudes are vehicle-scale
  and must NOT be transferred to walking â€” sign and structure only.**
- **Public-shelter aversion** â€” direction supported by Mesa-Arango et al.
  ...
  shares were friends/family 49â€“70%, hotel/motel 13â€“23%, **public shelters only
  2â€“5%**. Even among housed evacuees, formal shelters are a last resort.
- **Î²_d magnitude and range R** â€” class **A**, swept. R band 0.5â€“5 km, loosely
  ...
  rail) â€” **routine-trip evidence, not emergency-refuge evidence**, so it bounds
  rather than determines. Portland's two sites de facto served a citywide
  catchment, which argues for the upper end.

**Honest simplification:** with only 2â€“3 real shelters, MNL and
...
and travel time. Any explicit `Î²_health` term is class **A** with no source and
should default to 0.
...
(occupancy 40 and ~90; uptake ~6â€“7%), non-compliance floor (even among housed
populations 3â€“13% never comply with mandatory orders; ~46% total compliance in
the Kincade Fire GPS study), public-shelter aversion direction, negative distance
sign, lognormal delay family.

**Necessary assumptions (named scenario keys, each swept):** Î²â‚€, Î²â‚, Î²â‚‚ (fitted
then Â±50%), latent-class share Ï€, delay (Âµ, Ïƒ), walking range R (0.5â€“5 km),
Î²_d, word-of-mouth Îº (default 0), Î²_health (default 0). Seven to eight
sensitivity dimensions â€” tractable with a one-at-a-time or fractional-factorial
...
evacuate â€” an assumption the local record contradicts (~6â€“7% uptake, 65%
