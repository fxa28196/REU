...
**And where do they start?** Not on a grid. **The City of Portland's campsite-report feed holds 3,400 reports, which resolve to 3,317 distinct coordinates in the file.** Any one run seeds its residents at a subset of those — 2,981 places in the run on this slide — because placement is sampled, not exhaustive. **Every simulated person starts where somebody actually reported a campsite.** *(Presenter note: an earlier version of this line claimed the shortfall came from us de-duplicating repeat reports — that was wrong; no such step exists in the code, and duplicate coordinates explain only 83 of the 419 report-to-coordinate difference. Don't say it.)*
...
**If they ask:** *"After you split one corrupt ID into several places, do routes through them still connect?"* Yes, and we check it two ways. **The wormhole fix changed no connectivity — nothing became stranded by it.** And **a separate audit looks for any street segment whose two recorded ends are further apart than the segment's own recorded length — geometrically impossible, and the sign of a wormhole. Before the fix: 50. After: zero.**

**If they ask about the disconnected pieces:** a street network is not one connected blob, and this is a place where we corrected ourselves twice, so say it carefully. **First, we found freeway centrelines in the walking map — 2,636 features, six hundred and fourteen kilometres, including the Marquam and Fremont bridge decks, none of which a pedestrian may use. We removed them and re-ran everything.** The corrected walking map has **171 separate pieces, the largest holding 59,725 of its 88,100 intersections**, and **28 people in every scenario cannot reach any shelter — they start on a fragment whose only connection to the rest of the map was a freeway.** **Every headline percentage was unchanged by that correction — 30, 92 and 96 all survived to the digit — which is the strongest robustness evidence this study has.** *(Do NOT reuse the old dismissive answer about the remaining pieces all being tiny — the pre-fix graph's second piece was large and carried real demand, and the old 154/60,444/"16 people" numbers belong to the superseded map.)*
...
| | Scenario A: reality | Scenario B: more beds | Scenario C: same total, more doors |
|---|---|---|---|
| Facilities | 36 | 36 | 46 |
...
| Hours in unhealthy air, per person | 135.8 | 17.5 | 8.7 |
| Inhaled particulate, per person | 23,373 µg | 3,056 µg | 1,536 µg |
| Person-hours in unhealthy air, whole population | 928,918 | 119,973 | 59,200 |

**Say this before anyone adds a column, because they will:** **every column adds to 6,842. Got inside, plus turned away, plus twenty-eight people who cannot reach any shelter by any route. Those twenty-eight start on a piece of street network whose only link to the rest of the map was a freeway — legal to drive, illegal to walk — so once we removed freeways from the walking map, no amount of capacity and no re-placement reaches them, which is why the number is identical in all three scenarios.**
...
First: **on "turned away."** Those figures count *people* in a final state of having been refused everywhere. If you open our per-shelter file you will see a much bigger number — 17,167 door refusals in Scenario A — because one person is refused at many doors. **That column counts refusals at doors. The headline counts people.**

Second: **the real "got inside" numbers are probably lower than these.** The model assumes everyone knows the shelters exist. When we tested the 2020 configuration against the one occupancy observation we could find — **a *Street Roots* report from September 16th, 2020, which observed roughly 90 people at the Convention Center and 40 at Charles Jordan, about 130 of 198 beds, where the model fills 198 of 198 at both sites** — **the model over-admits by somewhere between one-and-a-half and fifteen times: the record is approximate, so 1.5–15.6× is the honest bracket, and 1.52× is only its most favourable edge.** We know a likely reason: a Portland State survey found **65% of unsheltered residents had never heard of the clean-air shelters.** We have not implemented that, and we flag it as a blocking assumption. **So every "got inside" figure here is optimistic — and optimistic in all three scenarios equally, which is why the comparison between them still holds.**

**One structural point worth volunteering, because it makes the causal story unusually clean.** The smoke field in this model is the same everywhere in the county at any given hour. **That is not laziness. There are only two regulatory monitors inside Multnomah County, and you cannot build a surface from two points.** The consequence matters: **no scenario can help by moving people into cleaner air, because there is no cleaner air anywhere. Everything you see here is a travel-time effect** — the headcount part comes from having more doors to try, and the shorter-walk part is the one place the siting algorithm earns credit. People get indoors sooner, and they spend less time walking — and walking is when they breathe hardest.
...
**The result I'd most want a policy audience to take away is on this slide, because it is the one you cannot see without a model of individuals: the residual access gap concentrates in the people who walk slowest, in every scenario and at every scale — and the one tested intervention that closes it costs nothing.**
...
| Group | Share | A: reality | B: more beds | C: same total, more doors | D: B + 10% reserve |
|---|---|---|---|---|---|
| Everyone | 100% | 30.1% | 91.6% | 96.0% | 91.6% |
| Walks without difficulty | 80.1% | 32.6% | 96.3% | 98.5% | 91.5% |
| **Has trouble walking** | **19.9%** | **20.1%** | **72.6%** | **86.0%** | **91.9%** |
| **Gap** | | **12.5 points** | **23.7 points** | **12.5 points** | **−0.4 points** |
...
**Say it slowly, because it is counter-intuitive: pouring 4,608 additional beds — that's 6,842 minus 2,234 — into the same buildings nearly doubles the equity gap.** From 12.5 points to 23.7. **Everyone improves in absolute terms. The fast walkers improve far more.**
...
**And the honest close on this slide.** In Scenario C, **14.0% of mobility-limited residents — about 190 people — are still outside.** More doors narrow the gap; they do not close it. **What closes it is the intake rule, and that costs nothing.**

There's one more piece of honesty here, and **it is spoken only — there is no row for it on the slide, so expect "show me."** **Relative inequality in inhaled dose keeps rising even as absolute harm collapses.** In Scenario A, mobility-limited people inhale about **1.2 times** what unimpaired people inhale. In B it's **6.3**. In C it's **6.9**.
...
**3. Everyone in the model knows every shelter exists, and everyone wants to go.** Neither is true. Sixty-five percent of surveyed unsheltered residents had never heard of these shelters. **This is why the model over-admits against the one real observation we have — by at least one and a half times, and possibly much more: the record is approximate, so the honest statement is a bracket, 1.5 to 15.6, not a settled number.**
...
**6. Beds are handed out in shuffle order, not by need — except in Scenario D, where we DID test prioritising the people who walk slowest.** Whoever's turn comes first in a given minute gets the bed; when beds are scarce, that rations shelter by walking speed. **Scenario D holds ten per cent of each shelter's spaces for mobility-limited arrivals and closes the equity gap at zero capital cost — what used to sit on this slide as an untested idea is now a tested result.**
...
No, and I want to be careful about this. **In absolute terms, mobility-limited people do vastly better in Scenario B than in Scenario A — from 20.1% getting inside to 72.6%.** Nobody is worse off. The point is narrower: the *benefit* of new capacity is not shared evenly, and a capacity-only evaluation would report 92% success and never see it. **The finding is that capacity and the intake rule should be decided together — Scenario D shows the rule alone closes the gap at zero cost — and that surplus dissolves the problem entirely, so the gap is a symptom of running exactly at capacity.**

**2. "Your model over-admits — somewhere between one-and-a-half and fifteen times — against the only real observation you have. Why should I believe any of it?"**
That's the right question, and the answer has two parts. First, **the direction is known and the reason is known** — we don't model whether people have heard of the shelters, and a local survey says 65% haven't. So we systematically over-count admissions; the record is approximate, which is why I quote a bracket of 1.5 to 15.6 rather than a settled number. Second, and more important: **that bias applies equally to every scenario.** The comparison between them — which is the entire finding — is far more robust than any absolute number. **I would not stand here and tell you exactly how many people get inside. I would tell you which comparisons survive: more doors admit more people than bigger buildings, the shortfall always lands on the slowest walkers, and a costless intake rule fixes that.**

**3. "Two air monitors for a whole county. Isn't your smoke field basically made up?"**
The smoke field is the same everywhere by design, and we say so in a dedicated audit. **With two monitors, any spatial interpolation would fit two points and manufacture gradients the data cannot support.** And the consequence actually helps the study: **since there is no clean air anywhere, no scenario can help by moving people into cleaner air. Every effect we report is a travel-time effect. That pins the mechanism down completely.** **The honest cost: we cannot say anything about neighbourhood-level differences in smoke, and I would not let anyone use this study for that.**
...
For this design, yes, and I can show why — but let me state it the careful way. **Change only the random starting point and the number admitted moves by at most 11 people; the smallest gap we claim between scenarios is 306. I put those two numbers side by side and let you judge — I deliberately don't divide them into a ratio, because seed spread only measures random-draw variability, not whether the model itself is right.** Nine replications is sufficient to establish an ordering separated by that margin. **And the B-to-C gap specifically is reproduced exactly by random site draws from the same candidate pool, so its tightness is evidence about dispersion, not about our optimiser. If the effects had been marginal, I'd need far more runs — and I'd tell you so.**
...
Carefully: **this is a modelling study, not a recommendation, and it predicts no health outcomes.** What it supports is one narrow, testable claim. **When shelter capacity is expanded for smoke response, where the new capacity goes matters roughly as much as how much of it there is — and the people most affected by that choice are the ones who walk slowest.** The concrete, checkable version: **in our reality scenario, 174 beds finish the event empty while 4,754 people are refused. That mismatch exists in the real inventory today, and it can be checked against real activation records without any model at all.** And the two cheapest levers the model found: **a ten-per-cent triage reserve costs nothing and closes the equity gap; twenty per cent surplus capacity dissolves the whole failure mode.** That's where I'd start.
...
**14. "Can I see the raw data? What does one run actually give you?"**
