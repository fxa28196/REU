// @ts-nocheck -- vendored third-party source; see the provenance note below.
 ...
 *
 * Source:      geographiclib-geodesic@2.2.0, files src/{Math.js, Geodesic.js, GeodesicLine.js, PolygonArea.js}
 * SHA-256:     0522063a3e9b98694e65b1e47472370ae94a97eab28c5488f9188c8272a2aa2b   (over those files, in that order, LF-normalised)
 *
 * The ONLY change from upstream: on every line that is not a comment, `Math.<m>` was
 * replaced by `Mx.<m>` for each m in
 *   sin, cos, atan, atan2, pow, exp, log, log1p, cbrt, atanh, hypot
...
// ===========================================================================================
// ==== BEGIN vendored geographiclib-geodesic src/Math.js
// ===========================================================================================
/*
 * Math.js
...
(function(
   ...
   * @constant {number}
   */
  m.epsilon = Mx.pow(0.5, m.digits - 1);
  /**
   ...
   */
  m.cbrt = Mx.cbrt || function(x) {
    var y = Mx.pow(Math.abs(x), 1/3);
...
// ==== END vendored geographiclib-geodesic src/Math.js

// ===========================================================================================
// ==== BEGIN vendored geographiclib-geodesic src/Geodesic.js
// ===========================================================================================
/*
 * Geodesic.js
...
(function(
  ...
  astroid = function(x, y) {
    ...
    if ( !(q === 0 && r <= 0) ) {
      ...
      if (disc >= 0) {
        ...
        ang = Mx.atan2(Math.sqrt(-disc), -(S + r3));
  ...
  g.Geodesic.prototype.InverseStart = function(sbet1, cbet1, dn1,
    ...
    if (shortline) {
      ...
      somg12 = Mx.sin(omg12); comg12 = Mx.cos(omg12);
    } else {
    ...
    if (shortline && ssig12 < this._etol2) {
      ...
      // Set return value
      vals.sig12 = Mx.atan2(ssig12, csig12);
      ...
      lam12x = Mx.atan2(-slam12, -clam12); // lam12 - pi
      if (this.f >= 0) {       // In fact f == 0 does not get here
        ...
        bet12a = Mx.atan2(sbet12a, cbet12a);
      ...
      if (y > -tol1_ && x > -1 - xthresh_) {
        ...
        somg12 = Mx.sin(omg12a); comg12 = -Mx.cos(omg12a);
  ...
  g.Geodesic.prototype.Lambda12 = function(sbet1, cbet1, dn1,
    ...
    vals.sig12 = Mx.atan2(Math.max(0, vals.csig1 * vals.ssig2 -
    ...
    // eta = omg12 - lam120
    eta = Mx.atan2(somg12 * clam120 - comg12 * slam120,
  ...
  g.Geodesic.prototype.InverseInt = function(lat1, lon1, lat2, lon2, outmask) {
    ...
    if (meridian) {
      ...
      // sig12 = sig2 - sig1
      sig12 = Mx.atan2(Math.max(0, csig1 * ssig2 - ssig1 * csig2),
    ...
    if (!meridian &&
      ...
      m12x = this._b * Mx.sin(sig12);
      if (outmask & g.GEODESICSCALE)
        vals.M12 = vals.M21 = Mx.cos(sig12);
      ...
      if (sig12 >= 0) {
        ...
        m12x = m.sq(dnm) * this._b * Mx.sin(sig12 / dnm);
        if (outmask & g.GEODESICSCALE)
          vals.M12 = vals.M21 = Mx.cos(sig12 / dnm);
        vals.a12 = sig12 / m.degree;
        ...
        for (tripn = false, tripb = false;; ++numit) {
          ...
          if (numit < maxit1_ && dv > 0) {
            ...
            if (Math.abs(dalp1) < Math.PI) {
              sdalp1 = Mx.sin(dalp1); cdalp1 = Mx.cos(dalp1);
        ...
        if (outmask & g.AREA) {
          // omg12 = lam12 - domg12
          sdomg12 = Mx.sin(domg12); cdomg12 = Mx.cos(domg12);
    ...
    if (outmask & g.AREA) {
      ...
      if (!meridian &&
        ...
        alp12 = 2 * Mx.atan2( somg12 * (sbet1*dbet2 + sbet2*dbet1),
...
// ==== END vendored geographiclib-geodesic src/Geodesic.js

// ===========================================================================================
// ==== BEGIN vendored geographiclib-geodesic src/GeodesicLine.js
// ===========================================================================================
/*
 * GeodesicLine.js
...
(function(
  ...
  l.GeodesicLine = function(geod, lat1, lon1, azi1, caps, salp1, calp1) {
    ...
    if (this.caps & g.CAP_C1) {
      ...
      s = Mx.sin(this._B11); c = Mx.cos(this._B11);
      // tau1 = sig1 + B11
  ...
  l.GeodesicLine.prototype.GenPosition = function(arcmode, s12_a12,
    ...
    if (arcmode) {
      ...
      ssig12 = Mx.sin(sig12); csig12 = Mx.cos(sig12);
    ...
    if (outmask & g.LONGITUDE) {
      ...
      omg12 = outmask & g.LONG_UNROLL ?
        E * (sig12 -
             (Mx.atan2(ssig2, csig2) -
              Mx.atan2(this._ssig1, this._csig1)) +
             (Mx.atan2(E * somg2, comg2) -
              Mx.atan2(E * this._somg1, this._comg1))) :
        Mx.atan2(somg2 * this._comg1 - comg2 * this._somg1,
    ...
    if (outmask & g.AREA) {
      ...
      vals.S12 = this._c2 * Mx.atan2(salp12, calp12) +
...
// ==== END vendored geographiclib-geodesic src/GeodesicLine.js

// ===========================================================================================
// ==== BEGIN vendored geographiclib-geodesic src/PolygonArea.js
// ===========================================================================================
/*
 * PolygonArea.js
...
// ==== END vendored geographiclib-geodesic src/PolygonArea.js
